...
module.exports = async ctx => {
  ...
  if (ctx.updateType === "callback_query") {
    ...
    if (locales[ctx.match[1]]) {
      ...
      await ctx.db.user.updateOne({ id: ctx.from.id }, { $set: { language_code: ctx.match[1] } })

      if (ctx.update.callback_query.message.chat.type !== "private")
        await ctx.db.chat.updateOne({ id: ctx.chat.id }, { $set: { language_code: ctx.match[1] } })
    ...
    Object.keys(locales).map((key) => {
      button.push(Markup.button.callback(locales[key].flag, `set_language:${key}`))
    })
