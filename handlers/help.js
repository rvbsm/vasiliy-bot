...
module.exports = async ctx => {
  if (ctx.chat.type === "private") return await ctx.reply(ctx.i18n.t("cmd.help.msg"))
  else {
    const chat = await ctx.db.chat.findOne({ id: ctx.chat.id })
    if (!chat) return
    if (ctx.message.text.split(' ')[1] === "default") {
      chat.commands_response.set("help",ctx.i18n.t("cmd.help.msg", { add: cfg.command.add, rem: cfg.command.rem, list: cfg.command.list }))
