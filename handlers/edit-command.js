...
module.exports = async (ctx) => {
  ...
  if (!command || ["help", "invite"].includes(command))
    return await ctx.replyWithHTML(
      ctx.i18n.t("cmd.add.error", { cmd: action })
    );
  ...
  if (action === "add") {
    ...
    const entities = ctx.message.entities
      .map((e) =>
        e.offset - 6 - command.length >= 0
          ? { ...e, offset: e.offset - 6 - command.length }
          : null
      )
      .filter(Boolean);
    ...
    if (!chat.commands_response.has(command))
      return await ctx.replyWithHTML(
        ctx.i18n.t("cmd.rem.error", { cmd: action })
      );
