module.exports = async (ctx) => {
  const vars = {
    first_name: ctx.message.new_chat_participant
      ? ctx.message.new_chat_participant.first_name
      : ctx.from.first_name,
    last_name: ctx.message.new_chat_participant
      ? ctx.message.new_chat_participant.last_name || ""
      : ctx.from.last_name || "",
    username: ctx.message.new_chat_participant
      ? ctx.message.new_chat_participant.username || ""
      : ctx.from.username || "",
    id: ctx.message.new_chat_participant
  ...
  const entities = (chat.commands_entities.get(command) || []).map(
    (e, idx, arr) => {
      ...
      n +=
        e.offset !== (arr[idx + 1] || { offset: -1 }).offset
          ? markdown.length - e.length
          : 0;
      return result;
    }
  );
  ...
  return await ctx.reply(
    message.replace(/\$(\w+)/g, (_, name) => vars[name] || "$" + name),
    { reply_to_message_id: ctx.message.message_id, entities: entities }
  );
};
